function e(e, o, n) {
    o && o(t(e));
    var r, i, s, c;
    if (r = e.getElems(), console.log("----\x3e msg.fromAccount: " + e.fromAccount + ", identifier: " + G.identifier), 
    e.fromAccount !== G.identifier) {
        for (var a in r) if (i = r[a]) if (s = i.getType(), console.log("0x256 - type: " + s), 
...
function o(e, o) {
    var n, r, t, s, c;
    switch ((n = e.getFromAccount()) || (n = ""), (r = e.getFromAccountNick()) || (r = n), 
    t = e.getSession().type(), s = e.getSubType(), t) {
      case R.SESSION_TYPE.C2C:
        switch (s) {
          case R.C2C_MSG_SUB_TYPE.COMMON:
            console.log(e.elems[0].content.text, "kkkkkk"), c = e.elems[0].content.text && e.elems[0].content.text.indexOf("csyzwfelab20180425hhhdfq") > -1 ? JSON.parse(e.elems[0].content.text) : i(e), 
            R.Log.warn("receive a new c2c msg: fromAccountNick=" + r + ", content=" + c);
            var a = {
                To_Account: n,
                LastedMsgTime: e.getTime()
            };
            R.c2CMsgReaded(a), console.error("收到一条c2c消息(好友消息或者全员推送消息): 发送人=" + r + ", 内容=" + c);
        }
        break;
...
function n(e, o, n, t, i) {
    R.login(e, o, n, function(o) {
        if (console.debug(o), console.debug(o), R.Log.info("webim登录成功", e), console.log("webim登陆成功", e), 
        G = e, r({
            ProfileItem: [ {
...
function i(e) {
    var o, n, r, t, i = "";
    o = e.getElems();
    for (var T in o) switch (n = o[T], r = n.getType(), t = n.getContent(), r) {
      ...
      default:
        R.Log.error("未知消息元素类型: elemType=" + r);
    }
...
function a(e) {
    var o = e.getImage(R.IMAGE_TYPE.SMALL), n = e.getImage(R.IMAGE_TYPE.LARGE), r = e.getImage(R.IMAGE_TYPE.ORIGIN);
    return n || (n = o), r || (r = o), "<img src='" + o.getUrl() + "#" + n.getUrl() + "#" + r.getUrl() + "' style='CURSOR: hand' id='" + e.getImageId() + "' bigImgUrl='" + n.getUrl() + "' onclick='imageClick(this)' />";
}
...
function f(e) {
    ...
    switch (o = e.getOpType(), n = e.getOpUserId(), o) {
      case R.GROUP_TIP_TYPE.JOIN:
        r = e.getUserIdList();
        for (var s in r) if (i += r[s] + ",", r.length > 10 && 9 == s) {
            i += "等" + r.length + "人";
            break;
        }
        i = i.substring(0, i.length - 1), i += "进入房间", i += ';{"type":' + o + ',"userIdList":"' + r.join(",") + '"}', 
        t = parseInt(t) + 1;
        break;

      case R.GROUP_TIP_TYPE.QUIT:
        i += n + "离开房间", i += ';{"type":' + o + ',"userIdList":"' + n + '"}', t > 0 && (t = parseInt(t) - 1);
        break;

      case R.GROUP_TIP_TYPE.KICK:
        i += n + "将", r = e.getUserIdList();
        for (var s in r) if (i += r[s] + ",", r.length > 10 && 9 == s) {
            i += "等" + r.length + "人";
            break;
        }
...
function I(e, o, n, r, t, i) {
    var s = "收到一条群系统消息: type=" + e + ", typeCh=" + o + ",群ID=" + n + ", 群名称=" + r + ", 内容=" + t + ", 时间=" + R.Tool.formatTimeStamp(i);
...
module.exports = {
    ...
    onBigGroupMsgNotify: function(o, n, r) {
        for (var t = o.length - 1; t >= 0; t--) {
            ...
            e(i, n, r), console.log("监听大群新消息", i);
        }
    },
    onMsgNotify: function(e, n) {
        console.log("***777***", e);
        for (var r in e) n(o(e[r]));
    },
    handlderMsg: o,
    sdkLogin: n,
    ...
    applyJoinBigGroup: function(e, o, n) {
        ...
        console.log("***applyJoinBigGroup***", e), N = e, h = null, R.applyJoinBigGroup(r, function(r) {
            r.JoinedStatus && "JoinedSuccess" == r.JoinedStatus ? (R.Log.info("进群成功"), console.log("进群成功"), 
            C = e, o && o({
                errCode: 0,
                callback: n
    ...
    imageClick: function(e) {
        var o = e.src.split("#"), n = o[0], r = o[1], t = o[2];
        R.Log.info("小图url:" + n), R.Log.info("大图url:" + r), R.Log.info("原图url:" + t);
    },
        ...
        C) {
            ...
            if (s.length < 1) console.error("发送的消息不能为空!"); else {
                ...
                u = "消息长度超出限制(最多" + Math.round(a / 3) + "汉字)"), c > a) console.error(u); else {
                    ...
                    var E, m, _, I, S, P = s.match(p);
                    if (!P || P.length < 1) E = new R.Msg.Elem.Text(s), console.log("***text_obj***", E), 
                    e.addText(E); else {
                        for (var N = 0; N < P.length; N++) (_ = s.substring(0, s.indexOf(P[N]))) && (E = new R.Msg.Elem.Text(_), 
                        e.addText(E)), I = R.EmotionDataIndexs[P[N]], R.Emotions[I] ? (m = new R.Msg.Elem.Face(I, P[N]), 
                        e.addFace(m)) : (E = new R.Msg.Elem.Text(P[N]), e.addText(E)), S = s.indexOf(P[N]) + P[N].length, 
                        s = s.substring(S);
                        s && (E = new R.Msg.Elem.Text(s), e.addText(E));
                    }
                    console.log(e), R.sendMsg(e, function(o) {
                        l == R.SESSION_TYPE.C2C && t(e), R.Log.info("发消息成功"), console.log("发消息成功"), r && r(t(e));
                    }, function(o) {
    ...
    sendCustomMsg: function(e, o, n, r) {
            ...
            g = "消息长度超出限制(最多" + Math.round(l / 3) + "汉字)"), a > l) alert(g); else {
                ...
                if (u.length < 1) console.error("发送的消息不能为空!"); else {
                    ...
                    g = "消息长度超出限制(最多" + Math.round(l / 3) + "汉字)"), a > l) console.error(g); else {
                        ...
                        e.addCustom(p);
                        var E, m, _, I, S, P = /\[[^[\]]{1,3}\]/gm, N = u.match(P);
                        if (!N || N.length < 1) E = new R.Msg.Elem.Text(u), e.addText(E); else {
                            for (var L = 0; L < N.length; L++) (_ = u.substring(0, u.indexOf(N[L]))) && (E = new R.Msg.Elem.Text(_), 
                            e.addText(E)), I = R.EmotionDataIndexs[N[L]], R.Emotions[I] ? (m = new R.Msg.Elem.Face(I, N[L]), 
                            e.addFace(m)) : (E = new R.Msg.Elem.Text(N[L]), e.addText(E)), S = u.indexOf(N[L]) + N[L].length, 
                            u = u.substring(S);
                            u && (E = new R.Msg.Elem.Text(u), e.addText(E));
                        }
                        R.sendMsg(e, function(o) {
                            y == R.SESSION_TYPE.C2C && t(e), R.Log.info("发自定义消息成功"), console.log("发自定义消息成功", e), 
                            r && r(t(e));
                        }, function(e) {
    ...
    sendC2CCustomMsg: function(e, o, n) {
        if (G.identifier) {
            var r = o.data || "", t = o.desc || "", i = o.ext || "", s = R.Tool.getStrBytes(r), c = R.MSG_MAX_LENGTH.C2C, a = "消息长度超出限制(最多" + Math.round(c / 3) + "汉字)";
            if (s > c) alert(a); else {
                var u = new R.Session(R.SESSION_TYPE.C2C, e, e, "", Math.round(new Date().getTime() / 1e3)), l = Math.round(4294967296 * Math.random()), g = Math.round(new Date().getTime() / 1e3), f = R.C2C_MSG_SUB_TYPE.COMMON, o = new R.Msg(u, !0, -1, l, g, G.identifier, f, G.identifierNick), d = new R.Msg.Elem.Custom(r, t, i);
    ...
    sendGroupLoveMsg: function() {
        if (G.identifier) if (C) {
            ...
            var e = Math.round(4294967296 * Math.random()), o = Math.round(new Date().getTime() / 1e3), n = R.GROUP_MSG_SUB_TYPE.LOVEMSG, r = new R.Msg(h, !0, -1, e, o, G.identifier, n, G.identifierNick), i = new R.Msg.Elem.Text("love_msg");
            r.addText(i), R.sendMsg(r, function(e) {
                y == R.SESSION_TYPE.C2C && t(r), R.Log.info("点赞成功");
            }, function(e) {
    ...
    showLoveMsgAnimation: m,
    initEmotionUL: function() {
        return;
    },
    showEmotionDialog: function() {
        openEmotionFlag ? (openEmotionFlag = !1, p()) : (openEmotionFlag = !0, E());
    },
    selectEmotionImg: _,
    ...
    getC2CHistoryMsgs: function(e, o, n, r, t, i, s) {
