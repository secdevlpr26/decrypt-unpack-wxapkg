...
Page({
    ...
    edit: function(t) {
        var e = parseInt(t.currentTarget.dataset.index), a = this.data.addressObjects[e].get("objectId");
        wx.navigateTo({
            url: "../add/add?objectId=" + a
        });
    },
    delete: function(e) {
        var a = this, s = parseInt(e.currentTarget.dataset.index);
        a.data.addressObjects[s];
        wx.showModal({
            title: "确认",
            content: "要删除这个地址吗？",
            success: function(e) {
                e.confirm && t.getJSON("/User/del_address/user_id/" + user_id + "/id/" + address_id, function(t) {
                    wx.showToast({
                        title: t.data.msg,
                        icon: "success",
                        duration: 2e3
