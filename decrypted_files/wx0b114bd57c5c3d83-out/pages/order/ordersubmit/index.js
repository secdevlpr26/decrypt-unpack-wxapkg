...
Page({
    ...
    useCoupon: function() {
        if (-1 != this.data.cpos) {
            ...
            a.total_hyj <= a.total_fee && (a.total_fee = a.total_fee - t), a.total_fee < 0 && (a.total_fee = 0), 
            this.setData({
                totalPrice: a
            });
        }
    ...
    formSubmit: function(t) {
        ...
        s.getJSON("/Cart/cart3/act/submit_order/user_id/" + e + "/address_id/" + a + "/user_money/" + o + "/pay_points/" + i + "/couponTypeSelect/" + r + "/coupon_id/" + u + "/couponCode/" + d, function(t) {
            if (1 == t.data.status) {
                var a = t.data.result;
                n.globalData.wxdata = t.data.data, n.globalData.order = t.data.order, console.log(t), 
    ...
    getCarts: function(a) {
        ...
        s.getJSON("/Cart/cart2/user_id/" + o, function(a) {
            ...
            var c = a.data.result.couponList, l = i.data.coupon;
            for (var p in c) l.push(c[p].name);
            i.setData({
                coupon: l,
                couponList: c
            }), i.initData();
