...
Page({
    ...
    loadOrder: function(t) {
        ...
        if (!i.isloading) {
            ...
            var e = i.data.recordcount, s = i.data.orderlist.length;
            if (e > s && this.data.falgAjax) {
                this.setData({
                    falgAjax: !1
                });
                var n = Math.ceil(s / i.data.pagesize) + 1;
                ...
                i.data.sortcol && "&sortcol=" + i.data.sortcol + "&sort=" + i.data.sort, a.sendRequest({
                    url: "/index.php?c=Front/WxApp/JsonApi&a=getFinance&page=" + n + "&pagesize=" + i.data.pagesize,
                    method: "GET",
                    success: function(a) {
                        if (i.isloading = !1, a.success) {
                            for (var t = 0; t < a.finance.FinanceList.yearmonth.length; t++) for (var e = 0; e < a.finance.FinanceList.yearmonth[t].list.length; e++) {
                                var s = a.finance.FinanceList.yearmonth[t].list[e].CrTime.split(" ")[0].split("-").slice(1, 3).join("-"), n = a.finance.FinanceList.yearmonth[t].list[e].CrTime.split(" ")[1].split(":").slice(0, 2).join(":");
                                a.finance.FinanceList.yearmonth[t].list[e].date = s, a.finance.FinanceList.yearmonth[t].list[e].time = n;
                            }
                            i.setData({
