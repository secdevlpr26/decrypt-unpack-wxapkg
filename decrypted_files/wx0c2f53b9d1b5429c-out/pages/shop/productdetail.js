...
Page({
    ...
    initData: function() {
        ...
        this.loadCartNum(), this.loadProduct(), this.setData({
            ...
            scrviewFlag: !1
        }), s.initCommonModules(), i.loadCouponList(!0, function(a) {
            t.setData({
                list: a.list
            });
        });
    ...
    doAttrChange: function(a, e) {
        ...
        var l = [];
        for (var r in n) {
            var d = this.findAttrValItemById(n[r]);
            if (d.AttrValBigImg) {
                var g = t.globalData.siteBaseUrl + d.AttrValBigImg;
                this.setData({
                    attrImg: g
                });
            }
            l.push(d.AttrValName);
        }
        this.setData({
    ...
    shareHfive: function() {
        ...
        wx.navigateTo({
            url: "./poster?productId=" + t.data.productInfo.ProductID + "&name=" + t.data.productInfo.Name + "&price=" + t.data.productInfo.Price + "&productImg=" + a[0] + "&SiteID=" + t.data.productInfo.SiteID
        });
    }
});