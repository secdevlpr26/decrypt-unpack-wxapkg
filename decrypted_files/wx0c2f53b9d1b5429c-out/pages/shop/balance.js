...
Page({
    ...
    loadOrder: function(t) {
        ...
        if (!e.isloading) {
            ...
            var i = e.data.recordcount, s = e.data.orderlist.length;
            if (i > s && this.data.falgAjax) {
                this.setData({
                    falgAjax: !1
                });
                var o = Math.ceil(s / e.data.pagesize) + 1;
                ...
                e.data.sortcol && "&sortcol=" + e.data.sortcol + "&sort=" + e.data.sort, a.sendRequest({
                    url: "/index.php?c=Front/WxApp/ShopApi&a=getFinance&page=" + o + "&pagesize=" + e.data.pagesize,
                    method: "GET",
                    success: function(a) {
