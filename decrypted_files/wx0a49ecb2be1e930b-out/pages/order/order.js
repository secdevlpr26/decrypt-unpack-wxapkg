...
Page({
    ...
    onLoad: function(o) {
        ...
        var r = this, s = r.isShowpay();
        r.getUserInfo(), r.setData({
            isShowPrePay: s
        });
        var n = [];
        n = "booking" == o.status ? wx.getStorageSync("bookingDataList" + t.globalData.autoId) : "51" == o.status || "52" == o.status || "3" == o.status || "6" == o.status ? wx.getStorageSync("groupProDataList" + t.globalData.autoId) : wx.getStorageSync("cartDataList" + t.globalData.autoId);
        for (var d = [], c = 0; c < n.length; c++) if (n[c].isSelected) {
            var l = n[c];
            r.data.transportFee[c] = (0).toFixed(2);
            for (var u = [], p = 0, m = 0; m < l.pro.length; m++) n[c].pro[m].isSelected && (p += parseFloat(n[c].pro[m].price) * parseInt(n[c].pro[m].count), 
            u.push(n[c].pro[m]));
            r.data.proPrice = r.data.proPrice.concat(p), n[c].pro = u, d.push(n[c]), r.data.totalPrice += r.data.transportFee[c], 
            r.data.branchInfoIds += n[c].branchInfoId + ",";
        }
        r.data.branchInfoIds = r.data.branchInfoIds + "-1,-2", n = d, r.setData({
